<template>
  <v-container>
    <!-- 슬라이드쇼 -->
    <v-carousel cycle hide-delimiter-background v-model="model" height="400px">
    ...
    <v-container fluid>
      <v-card>
        <v-tabs background-color="blue" fixed-tabs dark>
          <v-tab>카테고리1</v-tab>
          <v-tab>카테고리2</v-tab>
          <v-tab>카테고리3</v-tab>
          <v-tab>카테고리4</v-tab>
            ...
            <v-container fluid>
              <v-row>
                <v-col v-for="i in 4" :key="i" cols="12" md="3">
                  <v-img
                    :src="
                      `https://picsum.photos/500/300?image=${i * n * 5 + 10}`
                    "
                    :lazy-src="
